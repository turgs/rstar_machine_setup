...
# Generated by provision_vps.sh

# CRITICAL: Port must be set before Match blocks
Port $SSH_PORT
...
# Disable forwarding (not needed for Kamal)
...
# Modern cryptography only (OpenSSH 7.4+)
...
# User-specific overrides (Match blocks MUST be at the end)
# Deploy user: Keys only (production secure)
Match User $DEPLOY_USER
    PasswordAuthentication no
    PubkeyAuthentication yes

# Root user: Allow password for emergency access
Match User root
...
EOF
    ...
    if ! sshd -t 2>&1; then
        ...
        error "SSH configuration test failed - backup restored"
    fi
    ...
    # Verify SSH is actually listening on the configured port
    local RETRY=0
    while ! ss -tlnp | grep -q ":$SSH_PORT.*sshd" && [[ $RETRY -lt 10 ]]; do
        echo "  Waiting for SSH to bind to port $SSH_PORT..."
        sleep 1
        RETRY=$((RETRY + 1))
    done
    
    if ! ss -tlnp | grep -q ":$SSH_PORT.*sshd"; then
        echo "  ✗ SSH not listening on port $SSH_PORT!"
...
configure_supermicro_fans() {
    ...
    if ! echo "$BOARD_MFR" | grep -qi supermicro || ! echo "$BOARD_NAME" | grep -qiE "X1[0-3]|H1[0-3]"; then
        echo "⚠ Not a Supermicro X10-X13 board (${BOARD_MFR:-unknown} / ${BOARD_NAME:-unknown}), skipping fan control"
        mark_complete "configure_supermicro_fans"
        return
    fi
    
    log "Configuring Supermicro Fan Control (${BOARD_NAME})"
    ...
    echo "coretemp" >> /etc/modules-load.d/smfc.conf 2>/dev/null || true
    
    # Create smfc config
    mkdir -p /etc/smfc
    cat > /etc/smfc/smfc.conf << 'SMFCEOF'
# smfc configuration — auto-generated by provision_vps.sh
